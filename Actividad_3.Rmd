...
# Construir datos sin eliminar de más
datos_roc_log <- rotacion %>%
  mutate(prob_log = predict(modelo_log, type = "response")) %>%
  select(rotacion, prob_log)

# Eliminar solo NA necesarios
datos_roc_log <- datos_roc_log[complete.cases(datos_roc_log), ]

# Verificar clases
table(datos_roc_log$rotacion)

# ROC
roc_log <- roc(
  response = datos_roc_log$rotacion,
  predictor = datos_roc_log$prob_log,
  levels = c(0,1),
  direction = "<"
)
...
plot(
  ...
  col = "red",
  lwd = 2,
  main = "Curva ROC - Modelo logistico"
)

# Leyenda (esto es lo que querias)
legend(
  "bottomright",
  legend = paste("AUC =", round(auc_log, 3)),
  col = "red",
  lwd = 2,
  bty = "n"   # opcional: sin caja
)
#```
...
conf_df <- conf_df %>%
  mutate(
    Tipo = case_when(
      Predicho == 1 & Real == 1 ~ "Verdaderos positivos",
      Predicho == 0 & Real == 0 ~ "Verdaderos negativos",
      Predicho == 1 & Real == 0 ~ "Falsos positivos",
      Predicho == 0 & Real == 1 ~ "Falsos negativos"
    )
  )

ggplot(conf_df, aes(x = factor(Real), y = factor(Predicho), fill = Tipo)) +
  geom_tile(color = "white", size = 1.2) +
  geom_text(aes(label = Freq), size = 6, fontface = "bold") +
  scale_fill_manual(values = c(
    "Verdaderos positivos" = "#2E7D32",   # verde fuerte
    "Verdaderos negativos" = "#1E88E5",   # azul
    "Falsos positivos" = "#FB8C00",       # naranja
    "Falsos negativos" = "#E53935"        # rojo
  )) +
  labs(
    title = "Matriz de confusion del modelo",
    x = "Valor real",
    y = "Prediccion"
  ) +
  theme_minimal() +
...
generar_individuo_realista <- function(base) {
  
  nuevo <- data.frame(
    distancia_casa = sample(base$distancia_casa, 1),
    anos_experiencia = sample(base$anos_experiencia, 1),
    anos_acargo_con_mismo_jefe = sample(base$anos_acargo_con_mismo_jefe, 1),
    departamento = sample(base$departamento, 1),
    satisfacion_laboral = sample(base$satisfacion_laboral, 1),
    horas_extra = sample(base$horas_extra, 1)
  )
